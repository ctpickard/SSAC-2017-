...
game.information = read.csv('0021500601[CLE-HOU-1-15-16]_tracking-data.csv')
...
shot.sportvu.key = data.frame(matrix(nrow = nrow(shot.pbp.key) - 2, ncol = 8))
...
for(i in 1:nrow(shot.sportvu.key)){
  ...
  event.data = filter(game.information, 
                      quarter == shot.pbp.key$quarter[i + 2], 
                      game_clock >= shot.pbp.key$time[i + 2], 
                      game_clock < shot.pbp.key$time[i + 2] + look_back)
  ...
  if(nrow(event.data) == 0){
    ...
    shooter.location = filter(event.data, player_id == as.character(shot.pbp.key$shooter[i + 2]))
    ...
    for(j in 1:length(distance_threshold)){
      ...
      for(k in 1:nrow(ball.location)){
        ...
        if(distance_ball < distance_threshold[j] && ball.location$z_loc[k] > 7.5){
          ...
          shot.sportvu.key[i, 4] = as.character(shot.pbp.key$shooter[i + 2])
          shot.sportvu.key[i, 5] = shot.pbp.key$event_id[i + 2]
          shot.sportvu.key[i, 6] = shot.pbp.key$result[i + 2]
          shot.sportvu.key[i, 7] = shot.pbp.key$shot_distance[i + 2]
          shot.sportvu.key[i, 8] = as.character(shot.pbp.key$assister[i + 2])
          break
        }
      }
...
pass.sportvu.key = data.frame(matrix(nrow = nrow(shot.sportvu.key), ncol = 11))
colnames(pass.sportvu.key) = c('passer_Id', 'time_pass', 'shooter', 'time_receive', 'time_shot', 'quarter', 
                               'shot_result', 'assist_status', 'time_to_shoot', 'dribbles', 'shot_distance') 
...
for(i in 1:nrow(shot.sportvu.key)){
  ...
  if(length(pass.sequence) < 2){
    ...
    pass.sportvu.key[i, 10] = 0
    pass.sportvu.key[i, 11] = 0
    ...
    shooter.information = filter(ball.control.information, game_clock <= time.receive)
    dribble = 0
    index = 1
    
    while(index < nrow(shooter.information)){
      if(shooter.information$ball_height[index] < 2.0){
        dribble = dribble + 1
        
        while(shooter.information$ball_height[index] < 2.0){
          index = index + 1
        }
      } else{
        index = index + 1
    ...
    pass.sportvu.key[i, 10] = dribble
    pass.sportvu.key[i, 11] = shot.sportvu.key$Shot_Distance[i]
